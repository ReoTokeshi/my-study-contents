...
ローカル変数は型によらず、自分で初期化が必要。使う前に初期化してるか、初期化してなくても使わなければOK。初期化せず使うとコンパイルエラー。（フィールドや配列は自動初期化あり）
...
２次元配列は**１次元目の要素数は必須**。２次元目は書かないと要素数０。<br>
`int[][] a = new int[][3];　←エラー`  
...
- **メソッド**の場合・・  
オーバーライドにならない同シグネチャのメソッド定義は不可。なので、  
...
- **代替コンストラクタ**  
`this.`ですべてのコンポーネントを初期化しない場合、他のコンストラクタを呼び出して<u>全てのコンポーネントの初期化必須（finalのため）。</u><br>コンストラクタ呼び出しが入ってないとエラー。
- **コンパクトコンストラクタ**  
...
　ただし`var list = new ArrayList<>();`はOK（Objectになる）<br>
